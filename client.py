...
cluster = MongoClient("mongodb+srv://sai:kathika@cluster0.gknvq.mongodb.net/test?retryWrites=true&w=majority")
db = cluster["chatLogs"]
collection = db["test"]
...
def connect_to_server(name):
    print("*************IN connect_to_server() METHOD*************")
...
def receive_message_from_server(sck, m):
    print("*************IN receive_message_from_server() METHOD*************")
    while True:
...
def getChatMessage(msg):
    print("*************IN getChatMessage() METHOD*************")
    msg = msg.replace('\n', '')
    texts = tkDisplay.get("1.0", tk.END).strip()

    messageHistory.append((username,msg))
    # print(messageHistory)

    # post = { "name": username, "chatLog" : messageHistory}
...
def send_mssage_to_server(msg):
    print("*************IN send_mssage_to_server() METHOD*************")
    msg = msg.encode()
    client.send(msg)
    print(msg)
    if msg.decode() == "exit" or msg == b"exit":
